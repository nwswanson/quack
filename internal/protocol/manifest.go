...
func ParseSiteManifest(r io.Reader, size int64) (SiteManifest, error) {
	if size > MaxSiteManifestBytes {
		return SiteManifest{}, fmt.Errorf("site.yaml exceeds %d bytes", MaxSiteManifestBytes)
	}
	limited := io.LimitReader(r, MaxSiteManifestBytes+1)
	data, err := io.ReadAll(limited)
	if err != nil {
		return SiteManifest{}, fmt.Errorf("read site.yaml: %w", err)
	}
	...
	if err := dec.Decode(&manifest); err != nil {
		return SiteManifest{}, fmt.Errorf("invalid site.yaml: %w", err)
	}
	if manifest.Features.Database.Required && !manifest.Features.Database.Enabled {
		return SiteManifest{}, fmt.Errorf("database.required cannot be true when database.enabled is false")
	}
	return manifest, nil
}
