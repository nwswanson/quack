...
def _apply_serial_event(topic, payload, payload_text = ""):
    ...
    _append_debug_and_publish("serial_event", {
        "topic": topic,
        "device": alias,
        "kind": kind,
        "payload": payload,
    ...
    if kind == "read_error" or kind == "write_error" or kind == "disconnected":
        ...
        _save_state(state)
        _state_changed()
        _append_terminal_and_publish(_terminal("error", "! " + text))
        return
...
def on_message(ctx, msg):
    ...
    if msg_type == "open":
        ...
        _save_state(state)
        _state_changed()
        _append_debug_and_publish("open", {"device": alias, "by": ctx.conn_id})
    ...
    if msg_type == "close":
        ...
        _save_state(state)
        _state_changed()
        _append_debug_and_publish("close", {"device": alias, "by": ctx.conn_id})
