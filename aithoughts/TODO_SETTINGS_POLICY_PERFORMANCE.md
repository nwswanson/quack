...
Settings and policy resolution currently performs synchronous SQLite reads on request paths that need effective serving decisions.
...
- Static serving checks site serving status through the resolver before serving content.
...
`CurrentSiteServingStatus` currently asks the database for current site manifests and then finds the matching site in memory. That keeps the resolver API simple, but it is not the right long-term shape for high request volume.

The immediate concern is not correctness. It is avoidable read amplification on every static request.

## Near-Term Fix

Add a targeted database method for serving-status lookups, for example:

```go
GetCurrentSiteManifest(ctx context.Context, site string) (CurrentSiteManifest, bool, error)
```

Then update `CurrentSiteServingStatus` to load only the requested site instead of listing all current site manifests.
...
- Site serving status remains resolved through the resolver.
- `go test ./...` passes.
